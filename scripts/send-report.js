...
const NS = 'kristin2070-appleshop';
const BRANDS = [
  { key: 'line',     name: '點選 LINE 好友',  color: '#06C755' },
  { key: 'facebook', name: '點進 FB 粉絲團',      color: '#1877F2' },
  { key: 'google',   name: '點進 Google 評論',  color: '#EA4335' },
];
const DASHBOARD = 'https://kristin2070.github.io/appleshop/stats.html';
const LANDING   = 'https://kristin2070.github.io/appleshop/';
...
<table cellspacing="0" cellpadding="0" border="0" align="center" width="600" style="margin:24px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.08);">
  <tr>
    <td style="background:linear-gradient(135deg,#FFD500,#FFB800);padding:24px;border-bottom:6px solid #1d1d1f;">
      <h1 style="margin:0;font-size:22px;color:#1d1d1f;">📊 週報 · 燦坤五甲店 Apple Shop</h1>
  ...
  <tr>
    <td style="padding:24px;">
      ...
      <p style="margin-top:18px;color:#888;font-size:11px;text-align:center;line-height:1.6;">
        ...
        © 2026 燦坤五甲店 Apple Shop · 此報表由系統自動產生於 ${today.toLocaleString('zh-TW')}
      </p>
    </td>
  </tr>
</table>
...
(async () => {
  ...
  await transporter.sendMail({
    from: `"燦坤五甲店 Apple Shop 週報" <${process.env.GMAIL_USER}>`,
    to: process.env.REPORT_TO,
    subject: `📊 [燦坤五甲店AppleShop週報] ${r.week[0].getMonth()+1}/${r.week[0].getDate()}–${today.getMonth()+1}/${today.getDate()} 共 ${weekTotal} 次點擊`,
    html,
    attachments: [{
      filename: `燦坤五甲店AppleShop-週報-${dateLabel}.xlsx`,
      content: xlsx,
    }]
  });
